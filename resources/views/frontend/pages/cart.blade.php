                                ...
                                <tr>
                                                ...
                                                <div class="cart-item__thumb">
                                                    @if ($cartItem->item->preview_type == 'image')
                                                        <a href="product-details.html" class="link">
                                                            <img src="{{ asset($cartItem->item->preview_image) }}"
                                                                alt="" class="cover-img">
                                                        </a>
                                                ...
                                                <div class="cart-item__content">
                                                    <h6
                                                        ...
                                                        <a href="product-details.html"
                                                            class="link">{{ $cartItem->item->name }}</a>
                                                    </h6>
                                                    <span class="cart-item__price font-18 text-heading fw-500">Category:
                                                        <span
                                                            class="text-body font-14">{{ $cartItem->item->category->name }}
                                                            / {{ $cartItem->item->subCategory->name }}</span>
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <td class="price">
                                        @if ($cartItem->item->discount_price > 0)
                                            <span
                                                class="cart-item__price font-18 text-heading fw-500">${{ $cartItem->item->discount_price }}</span>
                                            <span
                                                class="cart-item__price font-18 text-heading fw-400 text-decoration-line-through"
                                                style="color: red ! important">${{ $cartItem->item->price }}</span>
                                        @else
                                            <span
                                                class="cart-item__price font-18 text-heading fw-500">${{ $cartItem->item->price }}</span>
                                            {{-- <span class="cart-item__totalPrice text-body font-18 fw-400 mb-0">$28.00</span> --}}
                                        @endif
                                    </td>
                                    <td class="delete_cart">
                                        <span class="cart-item-remove" data-id="{{ $cartItem->id }}"><i
                                                class="ti ti-x"></i></span>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="text-center">{{ __('No item found') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
