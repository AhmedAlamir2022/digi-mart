...
class RoleUserController extends Controller implements HasMiddleware
{
    ...
    {
        return [
            new Middleware('permission:show user roles', only: ['index']),
            new Middleware('permission:create user roles', only: ['create', 'store']),
            new Middleware('permission:edit user roles', only: ['edit', 'update']),
            new Middleware('permission:delete user roles', only: ['destroy']),
        ];
    }
    /**
    ...
    public function store(RoleUserStoreRequest $request): RedirectResponse
    {
        ...
        notyf()->success('UserRole created successfully.');
    ...
    public function edit(Admin $role_user): View|RedirectResponse
    {
        if ($role_user->roles()->first()?->name === 'super admin') {
            notyf()->error('You cannot edit this user');
    ...
    public function update(RoleUserUpdateRequest $request, Admin $role_user): RedirectResponse
    {
        ...
        notyf()->info('UserRole updated successfully.');
    ...
    {
        if ($role_user->roles()->first()?->name === 'super admin') {
            notyf()->error('You cannot delete this user');
        ...
        try {
            ...
            notyf()->info('Role User deleted successfully.');
